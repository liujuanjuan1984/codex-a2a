...
User-facing runtime entrypoints live in the released `codex-a2a` CLI.
...
The `Publish` workflow now separates build, PyPI publish, and GitHub Release
sync so maintainers have a safe recovery path when GitHub Release creation
fails after PyPI already succeeded.

- Normal tag pushes still build, smoke-test, publish to PyPI, and then sync the
  GitHub Release assets for the same tag.
- Manual `workflow_dispatch` runs require `tag_name` and default
  `publish_to_pypi` to `false`, which makes them safe for GitHub Release
  recovery without attempting a duplicate PyPI upload.
- To backfill a missing GitHub Release for an already-published tag, rerun the
  `Publish` workflow with the existing tag and leave `publish_to_pypi=false`.
  The workflow will rebuild artifacts, ensure the release exists, and upload
  only missing assets.
