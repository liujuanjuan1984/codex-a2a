...
class CodexClient:
    ...
    async def _ensure_started(self) -> None:
        async def initialize_client() -> None:
            ...
            await self._transport.send_json_message({"method": "initialized", "params": {}})

        await self._transport.ensure_started(
            resolve_cli_bin=lambda: resolve_cli_bin(self._cli_bin),
            read_stdout_loop=lambda: self._transport.read_stdout_loop(
                dispatch_message=self._dispatch_message
            ),
            read_stderr_loop=self._transport.read_stderr_loop,
    ...
    async def _handle_notification(self, message: dict[str, Any]) -> None:
        await self._stream_bridge.handle_notification(
            message,
            enqueue_stream_event=self._stream_bridge.enqueue_stream_event,
            get_or_create_tracker=self._stream_bridge.get_or_create_tracker,
        )
    ...
    async def _handle_server_request(self, message: dict[str, Any]) -> None:
        await self._interrupt_bridge.handle_server_request(
            message,
            send_json_message=self._transport.send_json_message,
            enqueue_stream_event=self._stream_bridge.enqueue_stream_event,
        )
