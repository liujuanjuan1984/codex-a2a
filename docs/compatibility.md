...
- Inbound auth is intentionally static and deployment-scoped. The current contract supports a legacy single bearer token, a legacy single Basic credential pair, or a static multi-credential registry with stable principals; it does not claim OAuth2, OIDC, or dynamic token introspection.
